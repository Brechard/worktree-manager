...
function JetBrainsBox({
  primary,
  secondary,
  letter,
  className,
  ...props
}: SVGProps<SVGSVGElement> & { primary: string; secondary: string; letter: string }) {
  return (
...
function PlaceholderIcon({
  ...
  className,
  ...props
}: SVGProps<SVGSVGElement> & { bg: string; letter: string }) {
  return (
    <svg {...props} viewBox="0 0 64 64" className={cn('text-white', className)}>
...
const EDITOR_ICON_BY_ID: Record<string, Icon> = {
  ...
  'code-insiders': VisualStudioCodeInsiders,
  vscodium: VSCodium,
  zed: Zed,
...
export function getEditorIcon(editorId: string | undefined): Icon | undefined {
  if (!editorId) return undefined
  return EDITOR_ICON_BY_ID[editorId]
}
